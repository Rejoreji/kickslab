    ...
    <div className='max-w-5xl mx-auto p-6 shadow-md rounded-md'>
        ...
        <form action="" onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
                ...
                <textarea name="description" id="" value={productData.description} onChange={handleChange} className='w-full border border-gray-300 rounded-md p-2' rows={4} required></textarea>
            </div>
            {/* Price */}
             <div className="mb-6">
                <label htmlFor="" className="block font-semibold mb-2">Product Price</label>
            ...
            <div className="mb-6">
                ...
                <input type="number" name='counInStock' value={productData.counInStock} onChange={handleChange} className='w-full border border-gray-300 rounded-md p-2' required />
            </div>
            {/* SKU */}
            <div className="mb-6">
            ...
            <div className="mb-6">
                ...
                <input type="text" name='sizes' value={productData.sizes.join(",")} onChange={(e) => setProductData({...productData,sizes: e.target.value.split(",").map((sizes) => sizes.trim()),})} 
                className='w-full border border-gray-300 rounded-md p-2'  />
            </div>
            {/* Colors */}
             <div className="mb-6">
                <label htmlFor="" className="block font-semibold mb-2">Colors (comma-separated)</label>
                <input type="text" name='colors' value={productData.colors.join(",")} onChange={(e) => setProductData({...productData,colors: e.target.value.split(",").map((colors) => colors.trim()),})} 
                className='w-full border border-gray-300 rounded-md p-2'  />
            </div>
            ...
            <div className="mb-6">
                ...
                <div className="flex gap-4 mt-4 ">
                    {productData.images.map((image,index) => (
                        <div key={index}>
                            <img src={image.url} alt={image.altText || "product image"} srcset="" className='w-20 h-20 object-cover rounded-md shadow-md' />
                        </div>
                    ))}
                </div>
            </div>
